...
def pre_registro(dados: PreRegistro, db: Session = Depends(get_db)):
    ...
    usuario = Usuario(
        ...
        status="ativo",
        perfil={
            "nome_loja": dados.nome_loja or "Hazak",
            "cidade": dados.cidade or "Manaus",
            "regime": "MEI",
        },
    )
    db.add(usuario)
    db.flush()
    # cliente padrão "Consumidor" da loja (valor default p/ quem não se identifica)
    db.add(Cliente(usuario_id=usuario.id, nome="Consumidor", padrao=True))
    db.commit()
    db.refresh(usuario)
